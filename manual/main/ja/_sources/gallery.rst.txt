...
カスタムソルバーやベイズ推論のワークフロー、および TTOpt (テンソルトレイン最適化) アルゴリズムを用いた最適化を示すチュートリアルも含まれています。

- **gpr** -- ガウス過程回帰: カスタムソルバーで未知の関数をモデル化し、その最小値を推定する例 (``minsearch``)
- **linreg_with_noise** -- モデルエビデンス (model evidence) を用いた線形回帰とノイズレベル推定の例 (``pamc``)
- **ttopt_benchmark** -- TTOpt・連続パラメータ: 解析的なベンチマーク関数の最小化
- **qubo** -- TTOpt・離散パラメータ: QUBO (制約なし二値二次最適化) 問題
- **mip** -- TTOpt・連続および離散パラメータ: 混合整数計画 (MIP) 問題

各例には、メインスクリプト、ODAT-SE 入力ファイル、サンプルデータ、およびチュートリアルが含まれています。
