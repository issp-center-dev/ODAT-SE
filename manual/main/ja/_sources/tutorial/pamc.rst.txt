...
詳細は「入力ファイル」の章および :doc:`../algorithm/pamc` を参照してください。
...
- ``Tmin``, ``Tmax`` はそれぞれ温度の下限・上限です(逆温度で指定する場合は ``bmin``/``bmax`` を用います)。
...
それぞれ書式は同じで、最初の2列がステップ数とプロセス内の walker (replica) 番号、3列目が(逆)温度、4列目が目的関数の値、5列目以降がパラメータです。
最後の2列は、 walker の重み (Neal-Jarzynski weight) と祖先(計算を開始したときのレプリカ)の番号です。
...
1列目は逆温度で、2・3列目は目的関数 :math:`f(x)` の期待値と標準誤差、4列目はレプリカの個数、5列目は分配関数の比の対数 :math:`\log(Z_n/Z_0)` (:math:`Z_0` は最初の温度点における分配関数)、6列目はモンテカルロ更新の採択率です。
...
``plot_result_2dmap.py`` を実行すると、全温度点のサンプルを読み込み、
温度 ``T`` で色分けした図 ``result_T.png`` （下図左）と、目的関数値 ``f(x)`` で色分けした図 ``result_fx.png`` （下図右）、
および両者を並べた ``result.png`` が作成されます。

.. code-block::

    $ python3 plot_result_2dmap.py

なお、 ``plot_result_2d.py`` は、引数に指定した ``result_T%.txt`` ファイルのサンプル点を
``f(x)`` の値で色分けした図を1枚作成するスクリプトです（デフォルトの出力先は ``res.png``）。
...
.. seealso::
   PAMC の出力からは、model evidence の計算や事後確率分布のヒストグラム作成などの解析ができます。
   ポスト処理ツールを用いた解析の流れは :doc:`../post/index` を参照してください。
