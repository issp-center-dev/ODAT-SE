...
メインプログラム用の入力ファイル ``input.toml`` について説明します。記述方法の詳細については「入力ファイル」の章および :doc:`../algorithm/mapper_mpi` を参照してください。
...
標準出力には探索の進捗状況が表示されます。
各グリッド点（メッシュ）で評価されたパラメータ ``x1``, ``x2`` とその関数値は、最終的に ``output/ColorMap.txt`` に出力されます。
今回の場合は、
