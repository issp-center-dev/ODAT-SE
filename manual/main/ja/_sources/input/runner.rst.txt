...
- ``A``
  ...
  説明: :math:`M \times N` の変換行列 :math:`A` ( :math:`N` 次元のパラメータ :math:`x` を :math:`M` 次元の :math:`y` に写します)。空のリストを渡した場合、単位行列とみなされます。
...
- ``b``
  ...
  説明: :math:`M` 次元の平行移動ベクトル :math:`b` 。空のリストを渡した場合、ゼロベクトルとみなされます。
...
- ``co_b``
  ...
  説明: 制約式の縦ベクトル :math:`b` を設定します。
        ...
        ``co_b = [[0], [-1]]`` のように縦ベクトル（リストのリスト）として指定してください（ ``co_b = [0, -1]`` のような1次元リストはエラーになります）。
