...
- ``nreplica_per_proc``
  ...
  総レプリカ数は「MPI プロセス数 × ``nreplica_per_proc``」で与えられます。
  レプリカ交換モンテカルロ法では温度点はレプリカごとに1つ割り当てられるため、``Tmin`` から ``Tmax`` の間に生成される温度点の数は総レプリカ数に一致します。
  使用する温度点の数を増やすには、MPI プロセス数または ``nreplica_per_proc`` を増やしてください。

- ``separate_T``

  形式: 真偽値。 (default: true)

  説明: モンテカルロステップのログを温度ごとに分割したファイル (``result_T*.txt``) に出力するかどうか。
...
以下のファイルでは、各レプリカの状態を追跡する単位を walker と呼びます（1プロセスあたり ``nreplica_per_proc`` 個）。

``RANK/trial.txt``
^^^^^^^^^^^^^^^^^^^^^
各レプリカについて、モンテカルロ更新で提案（試行）されたパラメータ候補と、対応する目的関数の値です。
1列目にステップ数、2列目にプロセス内の walker 番号、3列目にレプリカの温度、4列目に目的関数の値、5列目以降にパラメータが記載されます。
...
``RANK/result.txt``
^^^^^^^^^^^^^^^^^^^^^
各レプリカについて、採択・棄却判定を経て採択されたパラメータの系列と、対応する目的関数の値です（棄却された場合は直前の状態が維持されます）。
``trial.txt`` と同一の書式です。
...
``#`` は温度点の番号です。
ファイルの1列目はステップ数、2列目は全体での walker 番号、3列目は目的関数の値、 4列目以降は探索パラメータの値です。
...
:math:`p` を求めるアルゴリズムはいくつか提案されていますが、 ODAT-SE では Metropolis-Hastings 法 (MH法) を用います。
...
レプリカ交換モンテカルロ法 (replica exchange Monte Carlo) は焼戻し法をさらに発展させた手法で、並列焼戻し法 (parallel tempering) とも呼ばれます。
...
有限温度由来の「ぼやけ」がどうしても生まれるので、モンテカルロ法で得られた解を初期値として :doc:`minsearch` を実行することを推奨します。
