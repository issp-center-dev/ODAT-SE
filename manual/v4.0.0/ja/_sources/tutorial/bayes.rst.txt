...
ここでは、ベイズ最適化を用いて Himmelblau関数の最小化問題を解析する方法について説明します。
...
メインプログラム用の入力ファイル ``input.toml`` を作成します。記述方法の詳細については「入力ファイル」の章および :doc:`../algorithm/bayes` を参照してください。
...
.. note::
   ベイズ最適化エンジン (PHYSBO) は最大化問題を扱うため、標準出力の ``f(x)`` には目的関数の符号を反転した値が表示されます。
   ``output/BayesData.txt`` には元の目的関数の値が記録されます。
