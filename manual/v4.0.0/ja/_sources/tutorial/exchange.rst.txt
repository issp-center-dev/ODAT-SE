...
詳細は「入力ファイル」の章および :doc:`../algorithm/exchange` を参照してください。
...
- ``Tlogspace`` が ``true`` の場合、温度を対数空間で等分割します（省略時のデフォルト値は ``true`` です）。
...
なお、Himmelblau 関数には4つの大域的最小点があり、この実行ではそのうちの一つ :math:`(-2.805, 3.131)` 付近の解が得られています。
...
.. seealso::
   ...
   計算結果を解析するためのポスト処理ツールが用意されています。詳細は :doc:`../post/index` を参照してください。
