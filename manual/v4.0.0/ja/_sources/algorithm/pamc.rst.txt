...
- ``separate_T``
  ...
  説明: モンテカルロステップのログを温度ごとに分割して出力するかどうか。
...
``RANK/result_T#.txt``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
各温度点ごとに、モンテカルロサンプリングで生成されたパラメータと、対応する目的関数の値です。
...
1列目には逆温度が、2列目と3列目には目的関数の期待値およびその標準誤差が、4列目にはレプリカの総数が、5列目には規格化因子(分配関数)の比の対数
