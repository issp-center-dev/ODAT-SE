...
- ``score``
  ...
  説明: スコア関数（獲得関数, acquisition function）を指定するパラメータ。
...
- ``num_rand_basis``
  ...
  説明: 基底関数の数。0を指定した場合、Bayesian linear modelを利用しない通常のガウス過程回帰が行われます。
...
1列目にメッシュのインデックス、
2列目以降は ``[algorithm]`` セクションの ``label_list`` で定義された変数(省略時は ``x1``, ``x2``, ...)に入る値が入ります。
...
リスタート
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
実行モードは ``odatse`` コマンドの引数 ``--init``, ``--resume``, ``--cont`` で指定します
（Python から直接利用する場合はコンストラクタの引数 ``run_mode`` に対応します）。
各モードの動作は次のとおりです。
...
BO では目的関数 :math:`f(\vec{x})` を、評価が速く最適化のしやすいモデル関数(多くの場合ガウス過程) :math:`g(\vec{x})` で近似します。
:math:`g` は、あらかじめ適当に決められたいくつかの点(訓練データセット) :math:`\{\vec{x}_i\}_{i=1}^N` での目的関数の値 :math:`\{f(\vec{x}_i)\}_{i=1}^N` をよく再現するように訓練されます。
パラメータ空間の各点において、訓練された :math:`g(\vec{x})` の値の期待値およびその誤差から求められる「スコア」 (acquisition function) が最適になるような点 :math:`\vec{x}_{N+1}` を次の計算候補点として提案します。
...
PHYSBO は ``mapper`` のように、あらかじめ決めておいた候補点の集合に対して「スコア」を計算して、最適解を提案します。
候補点の集合を分割することで MPI 並列実行が可能です。
