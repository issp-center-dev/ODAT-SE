...
Other parameters, such as the convergence criteria used in the Nelder-Mead method, can be set in the ``[algorithm.minimize]`` section, although they are omitted here because the default values are used. For details, see the "Input file" chapter and :doc:`../algorithm/minsearch`.
