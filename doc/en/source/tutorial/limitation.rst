...
- ``do.sh``
  ...
  Script prepared to run all the calculations of this tutorial at once.
...
- ``min_list`` and ``max_list`` specify the lower bound and upper bound of the parameter space, respectively.

- ``step_list`` is the step length of one MC update (standard deviation of the Gaussian distribution).
...
In the current version, the constraint expression is defined as :math:`Ax+b>0` where :math:`x` is the :math:`N`-dimensional input parameter, :math:`A` is an :math:`M \times N` matrix, and :math:`b` is an :math:`M`-dimensional vector.
...
The figure shows the posterior probability distribution and the two lines :math:`x_{1} - x_{2} = 0`, :math:`x_{1} + x_{2} - 1 = 0`, and it is confirmed that the search is restricted to the range where :math:`x_{1} - x_{2} > 0`, :math:`x_{1} + x_{2} - 1 > 0`.
