...
- ``odatse.mpi.algrank()``: if the current process is in the algorithm subcommunicator, it returns the rank of the current process in the algorithm subcommunicator, otherwise it returns ``mpi4py``'s ``MPI.COMM_NULL``
