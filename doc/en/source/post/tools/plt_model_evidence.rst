...
**\-\-auto-focus**
    Sets a flag that applies a method for determining a suitable plot window for the model evidence data based on the maximum value of the model evidence. The tightness of the focused window is controlled by the ``--focus-factor`` option argument.
