...
メインプログラム用の入力ファイル ``input.toml`` について説明します。記述方法の詳細については「入力ファイル」の章および :doc:`../algorithm/random_search` を参照してください。
...
- ``seed`` はパラメータ生成に用いる擬似乱数のシードを指定します。
...
標準出力にはサンプリングの進捗状況が表示されます。
ランダムに生成したパラメータ ``x1``, ``x2`` と、各点で計算された関数値は、最終的に ``output/ColorMap.txt`` に出力されます。
今回の場合は、
