...
ターゲット関数は ``evaluate`` で定義されます。ここでは設計行列 :math:`X` を用いた残差平方和(2次損失)を使っており、:math:`y = a x` (``dimension = 1``、切片なし)や :math:`y = a_0 + a_1 x` (``dimension = 2`` かつ ``has_intercept = true``) のような多項式フィットを扱えます。
...
- :math:`a^* = 1.0066` (数値。dimension = 1 の単一係数)
...
.. figure:: ../../../common/img/linreg_with_noise/linear_data_gaussian_noise.png
        ...
        図2: 関数 :math:`y=2.5x` から生成し、レベル :math:`\sigma^*=1,3,5` のガウスノイズを付加した人工データ(``data_gen.py`` で生成。デフォルト出力は ``data.png``)。
...
モデルがノイズレベル :math:`\sigma=5` の場合の回帰プロットとモデルエビデンスプロットを以下に示します。フィット図は実行時の出力ディレクトリに ``linear_fit_with_noise.png`` として保存されます。

.. figure:: ../../../common/img/linreg_with_noise/gaussian_noise5.png
        :name: gaussian_noise5

        図3: ノイズレベル :math:`\sigma^*=5` のデータセットに対する線形回帰プロットとモデルエビデンスプロット。
