...
入力データのヘッダ行から、データファイルが温度(T)または逆温度(beta)のどちらで記録されているかを判別します。判別できない場合は温度(T)とみなし、Warningを表示します。
...
4. 各ファイルの処理:
   ...
   a. ファイルを読み込み
   b. コメント行から、温度と逆温度のどちらで記録されているかを判別
   c. レプリカ数が指定されている場合は最後の n 行を抽出
   d. レプリカ数が指定されていない場合は最後のステップの行を抽出
   e. データ変換処理(温度→逆温度、不要なカラムの削除)
   f. 結果を出力ファイルに書き込み(追記)
