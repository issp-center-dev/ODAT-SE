...
def plot_linear_fit_with_noise(
    xdata, ydata, a, noise_level, beta_opt, output_file, intercept=False
):
    ...
    ax.fill_between(
        ...
        alpha=0.15,
        color="red",
        label=f"$\\pm2\\sigma$ noise",
    )

    ax.set_xlabel("x", fontsize=12)
    ax.set_ylabel("y", fontsize=12)
    ax.set_title(
        f"Linear Regression with Optimal Noise Level\n"
        + f"$a$ = {a}, $\\beta_{{opt}}$ = {beta_opt:.2e}, $\\sigma$ = {noise_level:.4f}",
        fontsize=14,
    )
    ax.legend(loc="best")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Integrated linear regression and model evidence analysis"
    )
    # Basic parameters
    parser.add_argument(
        "--input",
    ...
    log_pdb = calc_log_pdb(
        beta,
        logz,
        np.asarray([n_data], dtype=np.int64),
    ...
    plot_linear_fit_with_noise(
        xdata,
        ydata,
        a,
        noise_level,
        beta_opt,
        output_file=fit_plot,
